...
def inven(combat = False):
    ...
    while inventory_status == True:
        ...
        if re.match(r'^(1|health)$', action.lower()) != None:
            ...
            print(bcolors.OKCYAN + '(2) <<Arrows>>' +  bcolors.ENDC)
            print(bcolors.OKCYAN + '(3) <Crossbow>>' +  bcolors.ENDC)
            print(bcolors.OKCYAN + '(4) <<Bandages>>' +  bcolors.ENDC)
            print(bcolors.OKCYAN + '(5) <<Knife>>' +  bcolors.ENDC)
            print(bcolors.OKCYAN + '(6) <<Cross>>' +  bcolors.ENDC)
            print(bcolors.OKCYAN + '(7) <<Steak>>' +  bcolors.ENDC)
            print(bcolors.OKCYAN + '(8) <<Shield>>' +  bcolors.ENDC)
            print(bcolors.OKCYAN + '(9) <<Cloth>>' +  bcolors.ENDC)
            print(bcolors.OKCYAN + '(10) <<pelt>>' +  bcolors.ENDC)
            print(bcolors.OKCYAN + '(11) <<Bloody Rag>>' +  bcolors.ENDC)
            print(bcolors.OKCYAN + '(12) <<Wings>>' +  bcolors.ENDC)
            print(bcolors.OKCYAN + '(13) <<Fangs>>' +  bcolors.ENDC)
            print(bcolors.OKCYAN + '(14) <<Meat>>' +  bcolors.ENDC)
...
def equip(x):
    ...
    if 'weapon' in equiped:
        ...
        player_stat_bonus['min_ATK_bonus'] = 0
        player_stat_bonus['max_ATK_bonus'] = 0
        player_stat_bonus['HOLY_bonus'] = 0
...
def attack_confirm(hit_chance, is_crit=False):
    ...
    if re.match(r'^(yes|ye|y|1)$', confirmation.lower()) != None:
        ...
        invalid()
        attack_confirm(hit_chance, is_crit=False)
...
while run:
    while menu:
        ...
        if re.match(r'^(1|new game|one|first)$', choice.lower()) != None:
            ...
            min_ATK = load_list[3][:-1]
            max_ATK = load_list[4][:-1]
            HOLY = load_list[5][:-1]
            LUCK = load_list[6][:-1]
            SPEED = load_list[7][:-1]
            BLOODY_RAG = load_list[8][:-1]
            GOLD = load_list[9][:-1]
            location = load_list[10][:-1]
            clear()
            draw()
