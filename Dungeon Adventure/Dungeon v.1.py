...
def second_floor_DR():
    print('The hallway on the second floor has stairs on the right going to the attic, and stairs on the left leading to the living room. There is also a fuse box.')
    action = input('?')
...
def basement():
    if 'power' in inventory:
        print('You see a ghoast at the center of the room. He hasn\'t noticed you.')
        action = input('?')
        if re.match(r'^(ghoast|talk|talk ghoast|talk to ghoast)$', action.lower()) != None:
            spooky_ghoast()
...
def second_floor_LR():
    print('The hallway on the second has stairs on the right going to the dining room, and stairs in front of you leading to the attic. There is also a fuse box.')
    action = input('?')
...
def spooky_ghoast():
    print('The ghoast appears to pocess the finnest tophat, while maintaining a moist mustash. He has a golden monocle and gives off a spooky atmosphere.')
    print('He turns around and notices you."OH! Sorry, didn\'t see you there. You must be the poor fellow who\'s been wandering this cursed house(it keeps locking the attic door even when i already unlocked iy...). Anyway, I bet you wanna get out of here eh? Well don\'t worry, I\'ve got the entrance key right here! All you have to do is answer a riddle and i\'ll give it to you. However if you get it wrong, i\'ll have to eat ya soul. Even spooky ghoast such as myself get hungry sometimes ya know!"')
    spooky_ghoast_riddle()

def spooky_ghoast_riddle():
