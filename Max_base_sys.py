...
enemies_list = ['wolf','bat', 'vampire','gypsie','slime']
...
def use(x):
    ...
    if inventory[x]['duration'] > 0:
        ...
        inven()
    if inventory[x]['attack'] > 0:
        player_stat['min_ATK'] += inventory[x]['attack']
        player_stat['max_ATK'] += inventory[x]['attack']
        inventory[x]['quantity'] = inventory[x]['quantity'] - 1
        print(bcolors.OKGREEN + 'You gained ({})attack power!\n'.format(inventory[x]['attack']) + bcolors.ENDC)
        inven()
    if inventory[x]['rot'] > 0:
        player_stat['HP'] = player_stat['HP'] -  inventory[x]['rot']
        player_stat['max_HP'] = player_stat['max_HP'] -  inventory[x]['rot']
        inventory[x]['quantity'] = inventory[x]['quantity'] - 1
        print(bcolors.WARNING + 'You lost ({})max hp!'.format(inventory[x]['rot']) + bcolors.ENDC)
        inven()
...
def inven(combat = False):
    ...
    while inventory_status == True:
        ...
        if re.match(r'^(1|health)$', action.lower()) != None:
            ...
            print(bcolors.OKCYAN + '(11) <<slime [{}]>>' .format(inventory['slime']['quantity'])+  bcolors.ENDC)
            print(bcolors.OKCYAN + '(12) <<Bloody Rag[{}]>>'.format(inventory['bloody rag']['quantity']) +  bcolors.ENDC)
            print(bcolors.OKCYAN + '(13) <<Wings [{}]>>'.format(inventory['wing']['quantity']) +  bcolors.ENDC)
            print(bcolors.OKCYAN + '(14) <<Fangs [{}]>>'.format(inventory['fang']['quantity']) +  bcolors.ENDC)
            print(bcolors.OKCYAN + '(15) <<Meat [{}]>>'.format(inventory['meat']['quantity']) +  bcolors.ENDC)
            ...
            if re.match(r'^(1|cooked meat)$', specification.lower()) != None:
                ...
                inven()
            elif re.match(r'^(11|slime)$', specification.lower()) != None:
                print(inventory['steak'])
                if inventory['steak']['quantity'] > 0:
                    ask = input(bcolors.OKGREEN + 'Would you like to use this item?' +  bcolors.ENDC)
                    if re.match(r'^(yes|y|yeah|sure|use|yep)$', ask.lower()) != None:
                        ...
                        inven()
            elif re.match(r'^(12|bloody rag)$', specification.lower()) != None:
                ...
                if inventory['bloody rag']['quantity'] > 0:
                    ...
                    if re.match(r'^(yes|y|yeah|sure|use|yep)$', ask.lower()) != None:
                        ...
                        inven()
            elif re.match(r'^(13|wings|wing)$', specification.lower()) != None:
                print(inventory['wing'])
                inven()
            elif re.match(r'^(14|fangs|fang)$', specification.lower()) != None:
                print(inventory['fang'])
                inven()
            elif re.match(r'^(15|meat|meats)$', specification.lower()) != None:
...
while intro:
    ...
    print("but epic")
    print('Max L')
    print("Original game by: Holly H, Matt L, Jill L, Kai YT\n Press <0> to continue...")
...
map = [['forest','forest','forest','mountain','mountain'], # y = 0
        ...
        ['plains','fields','plains','forest','edge']]  # y = 6
